...
*------------------------------------------------------------------------------------------------------------------------*
	...
	* ------------------------------------------------------------------------------------------------------------------ *
	...
	format  pop %20.0fc
	keep if year == 2005 | year == 2007 | year == 2008 | year == 2009
	...
	* ------------------------------------------------------------------------------------------------------------------ *
	...
	foreach variable in spt_ principal_effort_index teacher_tenure  { //principal_effort_index teacher_tenure 
		...
		foreach grade in 5 9 {
			...
			gen 	T2009_`name'A`grade'  = T2009*`variable'`grade'
			gen 	`name'A`grade'		  =`variable'`grade'
			format `name'A`grade' %12.2fc
		}
	}

	gen  	absenteeism_int5   = 				absenteeism_teachers 	  == 2 //teacher abseemteism is a big issue
	replace absenteeism_int5   = . if missing(absenteeism_teachers)
	gen T2009_absenteeism_int5 = T2009*absenteeism_int5
	
	gen  	absenteeism_int9   = 				absenteeism_teachers 	  == 2 //teacher abseemteism is a big issue
	replace absenteeism_int9   = . if missing(absenteeism_teachers)
	...
	* ------------------------------------------------------------------------------------------------------------------ *
	foreach grade in 5 9 {
		...
		label var  spt_intA`grade'						"Students per teacher"
		label var  spt_`grade'							"Students per teacher"
		label var  principal_intA`grade'				"Principal Effort Index"
		label var  principal_int`grade'					"Principal Effort Index"
		label var  absenteeism_int`grade'				"Teacher's absenteeism" 
		label var  T2009_absenteeism_int`grade'			"Shutdown versus teacher's absenteeism"
		label var  T2009_spt_intA`grade' 				"Shutdown versus students per teacher"
		label var  T2009_principal_intA`grade' 			"Shutdown versus principal's effort"
	}
	...
	forvalues x = 1/9 {
		...
		**Labels em ingles
		...
		label var incentive_talk5						"% of students whose parents incentive talk about school"
