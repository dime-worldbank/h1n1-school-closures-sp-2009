...
*=============================>>
	...
	**Dataset
		...
		foreach grade in 5 {
		clonevar stuptea`grade' 	   	 	= spt`grade' 						//just renaming the variables because their names were too long to store in pboots matrix after we run boottest
		...
		**The strategy is to restrict the sample of locally managed schools. The treatment group  -> schools in the 13 municipalities that opted to extend the winter break.
			...
			sort 	codschool year
				...
				*-------------------------------------------------------------------------------------------------------------------------------------------------------------------*
				...
				//model 13 interaction of ATT versus age difference between the youngest and the oldest of the class
				//model 14 interaction of ATT versus % of teachers with the correct degree to teach portuguese/math for 5th graders
				
				
					if `grade' == 5 & (`sub' == 2 | `sub' == 3) {
						foreach variable in stuptea prin absen dif_age_ { //
					...
					if `grade' == 5 & (`sub' == 2 | `sub' == 3) {
							reg 	`subject' T2009 T2009_adeq`subject' adeq`subject' 	 	  				 i.T i.codmunic i.year $controls2007 						if (year == 2007 | year == 2009) & share_teacher_management_program == 0  [aw = `weight'], cluster(codmunic)
							eststo 		model`model'`sub'`grade', title("Dif-in-dif")   
							mat_res, 	model(`model') sub(`sub') var1(T2009) var2(T2009_`variable'`grade') var3(`variable'`grade')  dep_var(`subject') grade(`grade')
							local 		model = `model' + 1
					}					
